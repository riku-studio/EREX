...
- `app/models/` / `app/services/`：预留数据模型与业务服务实现；`services/index_rules.py` 支持文件/数据库双源读取索引规则。
...
- 数据库仅在 `INDEX_RULE_SOURCE=db` 时需要；默认容器镜像未强制安装数据库驱动，启用数据库时请添加 asyncpg 并启动 compose `db` profile。
